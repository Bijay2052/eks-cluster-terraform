...
resource "aws_iam_role_policy_attachment" "amazon_secrets_manager_read_write" {
  count      = var.enabled && var.use_existing_aws_iam_instance_profile == false ? 1 : 0
...
resource "aws_security_group_rule" "ingress_bastion" {
  count                    = var.enabled && var.allow_bastion_ingress == true && var.use_existing_security_group == false ? 1 : 0
  description              = "Allow bastion host to communicate to worker nodes"
  from_port                = 0
  to_port                  = 65535
  protocol                 = "-1"
  security_group_id        = join("", aws_security_group.default.*.id)
...
module "aws_ssh_worker_key_pair" {
  source = "../../commons/ssh-key-pair"

  namespace = module.label.namespace
  stage     = module.label.stage
  name      = "eks-worker-key-pair"
...
module "aws_worker_secrets_manager" {
  source        = "../../commons/secrets-manager"
  enabled       = var.enabled
  namespace     = module.label.namespace
  stage         = module.label.stage
  name          = "ec2-worker-key-pair"
  secret_string = module.aws_ssh_worker_key_pair.private_key
}
...
module "autoscale_group" {
  ...
  key_name                                = module.aws_ssh_worker_key_pair.key_name
